...
public class PackagingOrchestrationService {
    ...
    private final String PATH = "/opt/fhir/distribution/";
    private final byte[] zipPackage = null;
    ...
    public PackagingOrchestrationService(HapiFhirServer hapiFhirServer,
    ...
    public FhirPackage packageMinimum(String id, String format) {
        this.format = format;
        this.id = id;
    ...
    public byte[] packageForDistribution(String id) {
        ...
        if (optionalLibrary.isPresent()) {
            ...
            return "Primary Library not Found".getBytes();
        }
...
//             options.addProfile("http://build.fhir.org/ig/HL7/cqf-measures/branches/R4_Lift/StructureDefinition-library-bundle-cqfm.json");
    ...
    private void writeLibraryCQLandELM(Library library) {
    ...
    private void createDirectory() {
        ...
        if (!directory.exists()) {
            if (!directory.mkdir()) {
                throw new UncheckedIOException(new IOException("Cannot make directory: " + directoryName));
            }
        }
