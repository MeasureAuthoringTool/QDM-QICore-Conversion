...
import static gov.cms.mat.fhir.services.cql.parser.CqlUtils.areValidAscendingIndexes;
import static gov.cms.mat.fhir.services.cql.parser.CqlUtils.chomp1;
import static gov.cms.mat.fhir.services.cql.parser.CqlUtils.indexOf;
import static gov.cms.mat.fhir.services.cql.parser.CqlUtils.isValidVersion;
import static gov.cms.mat.fhir.services.cql.parser.CqlUtils.nextBlock;
import static gov.cms.mat.fhir.services.cql.parser.CqlUtils.nextCharMatching;
import static gov.cms.mat.fhir.services.cql.parser.CqlUtils.nextCharNotMatching;
import static gov.cms.mat.fhir.services.cql.parser.CqlUtils.nextNonWhitespace;
...
import static gov.cms.mat.fhir.services.cql.parser.CqlUtils.parseNextLine;
import static gov.cms.mat.fhir.services.cql.parser.CqlUtils.parsePrecedingComment;
import static gov.cms.mat.fhir.services.cql.parser.CqlUtils.removeCQLLineComments;
import static gov.cms.mat.fhir.services.cql.parser.CqlUtils.removeCqlBlockComments;
import static gov.cms.mat.fhir.services.cql.parser.CqlUtils.removeLastCqlBlockComment;
import static gov.cms.mat.fhir.services.cql.parser.CqlUtils.startsWith;
...
@Service
@Slf4j
public class DefaultCqlParser implements CqlParser {
        ...
        if (areValidAscendingIndexes(endFirstLine)) {
            ...
            if (!firstLine.startsWith(LIB_TOKEN)) {
                ...
                if (words.length == 4) {
                    ...
                    throw new CqlParseException("Invalid library encountered: " + firstLine + ". Verify the version is a valid mat version, e.g. '1.0.000'.");
                }
            }
        } else {
            throw new CqlParseException("First line did not start with " + LIB_TOKEN);
        }
    }
    ...
    private void processIncludeLibs(String cql, CqlVisitor v) {
        ...
        parseRepeatingLine(cql, INCLUDE_TOKEN, s -> {
            ...
            if (words.length != 6) {
                throw new CqlParseException("Invalid include encountered: " + s);
            }
            String name = words[1];
            ...
            v.includeLib(name, version, words[5], "FHIR", BaseProperties.LIBRARY_FHIR_VERSION);
        });
    }
    ...
    private void processValueSets(String cql, CqlVisitor v) {
        parseRepeatingLine(cql, VALUESET_TOKEN, s -> {
            ...
            if (areValidAscendingIndexes(type, uri)) {
                ...
                throw new CqlParseException("Invalid valueset encountered: '" + s);
            }
        });
    }
    ...
    private void processCodes(String cql, CqlVisitor v) {
        parseRepeatingLine(cql, CODE_TOKEN, s -> {
            ...
            if (areValidAscendingIndexes(name, code, from, codeSystem)) {
                ...
                throw new CqlParseException("Invalid code encountered: " + s);
            }
        });
    }
    ...
    private void processParameters(String cql, CqlVisitor v) {
        ...
        while ((nextDefineStart = indexOf(cql, PARAMETER_TOKEN, parsed)) > 0) {
            ...
            if (areValidAscendingIndexes(nameResult.getEndIndex())) {
                ...
                throw new CqlParseException("Invalid parameter type encountered around index " +
                        nextDefineStart);
            }
    ...
    private void processContext(String cql, CqlVisitor v) {
        ...
        if (StringUtils.isNotBlank(context)) {
            ...
            if (words.length == 2) {
                ...
                throw new CqlParseException("Invalid context encountered: " + context);
            }
        }
    }
        ...
        while ((nextDefineStart = indexOf(cql, DEFINE_TOKEN, parsed)) > 0) {
            if (!isDefineFunction(cql, nextDefineStart)) {
                ...
                if (areValidAscendingIndexes(name.getEndIndex(), colon, logic.getEndIndex())) {
                    ...
                    throw new CqlParseException("Encountered invalid define around index: " +
                            nextDefineStart);
                }
    ...
    private List<FunctionArgument> parseArguments(String argumentString) {
        ...
        var result = new ArrayList<gov.cms.mat.fhir.services.cql.parser.FunctionArgument>();
        ...
        while (parsed != -1 &&
            ...
            if (areValidAscendingIndexes(firstSpace, argType.getEndIndex())) {
                gov.cms.mat.fhir.services.cql.parser.FunctionArgument argument = new gov.cms.mat.fhir.services.cql.parser.FunctionArgument(argName, argType.getString());
                result.add(argument);
            } else {
