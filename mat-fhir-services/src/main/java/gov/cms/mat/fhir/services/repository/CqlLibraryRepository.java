...
public interface CqlLibraryRepository extends JpaRepository<CqlLibrary, String> {
    @Query("select a from CqlLibrary a where a.libraryModel = \'FHIR\' and a.measureId is null and a.draft = false order by a.lastModifiedOn asc")
    List<CqlLibrary> getAllVersionedCqlFhirLibs();
