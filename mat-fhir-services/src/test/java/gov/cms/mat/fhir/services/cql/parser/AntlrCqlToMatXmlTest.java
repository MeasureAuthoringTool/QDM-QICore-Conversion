...
public class AntlrCqlToMatXmlTest {
    ...
    public void mockSpreadsheet() {
        ...
        map.put("urn:oid:2.16.840.1.113883.6.1", new CodeSystemEntry("urn:oid:2.16.840.1.113883.6.1", "http://loinc.org", "LOINC"));
    ...
    public String loadCqlResource(String cqlResource) throws IOException {
        try (InputStream i = AntlrCqlToMatXmlTest.class.getResourceAsStream(CQL_TEST_RESOURCES_DIR + cqlResource)) {
    ...
    public void testFhirHelpers() throws Exception {
        ...
        assertThat("if period is null then\n" +
                "        null\n" +
                "    else\n" +
                "        Interval[period.\"start\".value, period.\"end\".value]", Matchers.equalToCompressingWhiteSpace(destination.getCqlFunctions().get(0).getLogic()));
        ...
        assertThat("if quantity is null then\n" +
                "        null\n" +
                "    else\n" +
                "        System.Quantity { value: quantity.value.value, unit: quantity.unit.value }", Matchers.equalToCompressingWhiteSpace(destination.getCqlFunctions().get(1).getLogic()));
        ...
        assertThat("if range is null then\n" +
                "        null\n" +
                "    else\n" +
                "        Interval[ToQuantity(range.low), ToQuantity(range.high)]", Matchers.equalToCompressingWhiteSpace(destination.getCqlFunctions().get(2).getLogic()));
...
//                "        }", Matchers.equalToCompressingWhiteSpace(destination.getCqlFunctions().get(3).getLogic()));
        ...
        assertThat("if concept is null then\n" +
                ...
                "        }", Matchers.equalToCompressingWhiteSpace(destination.getCqlFunctions().get(4).getLogic()));
...
//        assertEquals(232, destination.getCqlFunctions().size());
    ...
    public void testAudltOutpatientEncounters() throws Exception {
        ...
        assertThat("Interval<DateTime> default Interval[@2019-01-01T00:00:00.0, @2020-01-01T00:00:00.0)", Matchers.equalToCompressingWhiteSpace(destination.getCqlParameters().get(0).getLogic()));
    ...
    public void testHospiceLib() throws Exception {
        ...
        assertThat("exists (\n" +
                ...
                "    )", Matchers.equalToCompressingWhiteSpace(destination.getCqlFunctions().get(0).getLogic()));
    }
    ...
    public void testDefinitionsComment() throws Exception {
        ...
        assertThat(" testpopulation comment ", Matchers.equalToCompressingWhiteSpace(definition3.getCommentString()));
        assertEquals("true\n" +
                "\n" +
